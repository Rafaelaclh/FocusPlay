...
const FIREBASE_CONFIG = {
  apiKey: "AIzaSyC4yKtVY8fmwBUFYr3AVI-O-au_jYAxbd4",
  authDomain: "focusplay-1883d.firebaseapp.com",
  projectId: "focusplay-1883d",
  storageBucket: "focusplay-1883d.firebasestorage.app",
  messagingSenderId: "538185228423",
  appId: "1:538185228423:web:00f3b1186ec6d2a3c91822",
};
...
try {
  if (FIREBASE_CONFIG.apiKey && FIREBASE_CONFIG.apiKey !== "SUA_API_KEY") {
    ...
    console.log(
      "⚡ Modo offline (localStorage) — configure Firebase para cloud",
    );
  }
} catch (e) {
...
const LS = {
  get: (k, def = null) => {
    try {
      const v = localStorage.getItem("fp_" + k);
  ...
  set: (k, v) => {
    try {
      localStorage.setItem("fp_" + k, JSON.stringify(v));
    } catch {}
  },
  del: (k) => {
    try {
      localStorage.removeItem("fp_" + k);
    } catch {}
  },
};
...
const PLANTS = [
  {
    ...
    seedCost: 0,
    unlockLevel: 1,
    desc: "Cresce rápido, rende pouco. Boa para começar.",
  },
  {
    id: "carrot",
    emoji: "🥕",
  ...
  },
  {
    ...
    seedCost: 50,
    unlockLevel: 3,
    desc: "Precisa de atenção. Qualidade cai se você sair.",
  },
  {
    id: "pumpkin",
    emoji: "🎃",
  ...
  },
  {
    ...
    seedCost: 150,
    unlockLevel: 5,
    desc: "Progresso regride se você sair.",
  },
  {
    id: "melon",
    emoji: "🍈",
    ...
    seedCost: 200,
    unlockLevel: 6,
    desc: "Alta recompensa, alto risco.",
  },
  {
    id: "truffle",
    emoji: "🍄",
    ...
    seedCost: 400,
    unlockLevel: 8,
    desc: "MORRE se você sair durante a sessão!",
  },
  {
    id: "crystal",
    emoji: "💎",
    ...
    seedCost: 600,
    unlockLevel: 10,
    desc: "A planta mais valiosa. Extremamente frágil.",
  },
];
  ...
  expansions: [
    {
      ...
      icon: "🪣",
      price: 200,
      desc: "Plante 4 plantas por sessão",
      effect: "slots",
      val: 4,
    },
    {
      ...
      icon: "🪣",
      price: 400,
      desc: "Plante 5 plantas por sessão",
      effect: "slots",
      val: 5,
    },
    {
      ...
      icon: "🪣",
      price: 700,
      desc: "Plante 6 plantas por sessão",
      effect: "slots",
      val: 6,
    },
    {
      ...
      icon: "🏡",
      price: 1500,
      desc: "+50% rendimento de todas as plantas",
      effect: "greenhouse",
      val: 1.5,
    },
  ],
  upgrades: [
    {
      ...
      icon: "🌍",
      price: 300,
      desc: "+20% rendimento de todas as plantas",
      effect: "soil",
      val: 1.2,
    },
    {
      ...
      icon: "🌏",
      price: 600,
      desc: "+50% rendimento (acumulativo)",
      effect: "soil",
      val: 1.5,
    },
    {
      ...
      icon: "🧪",
      price: 100,
      desc: "Acelera 1 planta na próxima sessão",
      effect: "fertilizer",
      val: 1,
      consumable: true,
      qty: 3,
  ...
  cosmetics: [
    {
      ...
      icon: "🐱",
      price: 500,
      desc: "Seu pet fica na fazenda te acompanhando",
    },
    {
      id: "pet_dog",
      name: "Cachorro",
      icon: "🐶",
      price: 500,
      desc: "Fiel companheiro de estudos",
    },
    {
      id: "pet_fox",
      name: "Raposa",
    ...
    },
    {
      ...
      icon: "⭐",
      price: 300,
      desc: "Decoração brilhante para a fazenda",
    },
    {
      id: "deco_rain",
      name: "Chuva",
      icon: "🌧️",
      price: 300,
      desc: "Efeito de chuva na fazenda",
    },
  ],
};
...
const DAILY_CHALLENGES = [
  ...
  },
  {
    ...
    icon: "🔥",
    name: "Sem parar",
    desc: "Não saia durante 1 sessão inteira",
    target: 1,
    xpReward: 40,
    coinReward: 20,
  },
  ...
  },
  {
    ...
    icon: "🎯",
    name: "Foco longo",
    desc: "Complete uma sessão de 45 min",
    target: 45,
    xpReward: 80,
    coinReward: 50,
  },
  ...
  if (useFirebase && GS.uid) {
    try {
      updateDoc(doc(db, "users", GS.uid), {
        ...
        totalFocusMins: GS.totalFocusMins,
        rankScore: GS.rankScore,
        rankWeekly: GS.rankWeekly,
        name: GS.name,
        avatar: GS.avatar,
...
async function doRegister() {
  ...
  if (pass.length < 6) {
    setAuthError("Senha muito curta (mín. 6 chars)");
    return;
  }
  if (useFirebase) {
    try {
...
async function doForgotPassword() {
  ...
  const user = users[email];
  if (!user) {
    errorEl.textContent = "Email não cadastrado";
    return;
  }
  ...
  if (EMAILJS_SERVICE_ID === "SEU_SERVICE_ID") {
    ...
    console.warn(
      "[FocusPlay] Configure o EmailJS no app.js para envio de emails. Veja as instruções nos comentários da função doForgotPassword().",
    );
    closeForgotPassword();
    showToast(
      "⚠️ EmailJS não configurado. Veja o console para instruções.",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    closeForgotPassword();
    showToast(
      "✅ Senha enviada para " + email + "! Verifique sua caixa de entrada.",
      "success",
    );
  } catch (e) {
...
function updateHUD() {
  ...
  const pct = Math.min(
    100,
    ((GS.xp - cur.xpNeeded) / (next.xpNeeded - cur.xpNeeded)) * 100,
  );
  document.getElementById("xp-fill").style.width = pct + "%";
  document.getElementById("xp-text").textContent =
    `${GS.xp} / ${next.xpNeeded} XP`;
  document.getElementById("hero-level-badge").textContent =
    `Nv.${GS.level} ${cur.emoji}`;
  document.getElementById("hero-name").textContent = GS.name;

  document.getElementById("hs-sessions").textContent = GS.totalSessions;
  document.getElementById("hs-focus").textContent =
    Math.floor(GS.totalFocusMins / 60) + "h";
  ...
  document.getElementById("streak-msg").textContent =
    GS.streak > 0
      ...
      : "Faça uma sessão hoje para começar";
...
function setMode(mode) {
  ...
  document
    ...
    .forEach((t, i) =>
      t.classList.toggle("active", (mode === "solo" ? 0 : 1) === i),
    );
  document.getElementById("arena-badge-wrap").style.display =
    mode === "arena" ? "" : "none";
  saveState();
}
...
function toggleTimer() {
  GS.timerRunning ? pauseTimer() : startTimer();
}
...
function sessionComplete() {
  ...
  saveState();
  showToast(
    `🎉 Sessão completa! +${xpGain} XP, +${coinGain} moedas`,
    "success",
  );
  ...
  document.getElementById("timer-mode-label").textContent =
    GS.timerPhase === "long_break" ? "PAUSA LONGA" : "PAUSA CURTA";
  document.getElementById("active-session-banner").style.display = "none";
  updateTimerDisplay();
}
...
function farmSlotClick(i) {
  ...
  if (!plot) {
    ...
    showToast(
      `${PLANTS.find((p) => p.id === plot.plantId).emoji} Crescendo... ${Math.floor((plot.progress / plot.totalNeeded) * 100)}%`,
      "warning",
    );
  }
}
...
function closeSeedPicker() {
  document.getElementById("seed-picker").classList.remove("open");
}
...
function plantSeed(plantId) {
  ...
  if (!plant || (GS.inventory[plantId] || 0) <= 0) {
    showToast("Sem sementes!", "error");
    return;
  }
  GS.inventory[plantId]--;
  GS.farmPlots[currentPickSlot] = {
    plantId,
    name: plant.name,
    progress: 0,
    totalNeeded: plant.sessionMins,
    quality: 100,
    ready: false,
  };
  GS.totalPlants++;
  ...
  saveState();
  showToast(
    `🌱 ${plant.name} plantada! Inicie uma sessão para crescer.`,
    "success",
  );
}
...
function renderShopSection(elemId, type) {
  ...
  if (type === "seeds") {
    PLANTS.forEach((plant) => {
      ...
      const count = GS.inventory[plant.id] || 0;
      const canBuy =
        GS.coins >= plant.seedCost && GS.level >= plant.unlockLevel;
      el.innerHTML += `
        <div class="shop-item">
...
function buyItem(type, id) {
  if (type === "seed") {
    ...
    GS.coins -= item.price;
    if (item.consumable) {
      GS.purchased[id + "_qty"] = (GS.purchased[id + "_qty"] || 0) + item.val;
    } else {
      GS.purchased[id] = true;
    }
    if (item.effect === "slots")
      GS.farmSlots = Math.max(GS.farmSlots, item.val);
    if (item.effect === "greenhouse") GS.hasGreenhouse = true;
    if (item.effect === "soil")
      GS.soilMultiplier = Math.max(GS.soilMultiplier, item.val);
...
function switchRankTab(tab) {
  ...
  document
    ...
    .forEach((t, i) =>
      t.classList.toggle("active", ["daily", "weekly", "all"][i] === tab),
    );
  renderRanking();
}
...
async function renderRanking() {
  ...
  if (useFirebase) {
    try {
      ...
      players = (await getDocs(q)).docs.map((d) => ({
        ...d.data(),
        uid: d.id,
      }));
    } catch {
...
function checkLevelUp() {
  ...
  if (newLevel > GS.level) {
    ...
    document.getElementById("lu-level").textContent =
      `${newLevel} — ${lvl.name}`;
...
function updateChallengeProgress(id, amount) {
  ...
  if (GS.challengeProgress[id] >= ch.target && prev < ch.target) {
    GS.xp += ch.xpReward;
    GS.coins += ch.coinReward;
    showToast(
      `🎯 Desafio "${ch.name}" completo! +${ch.xpReward} XP`,
      "success",
    );
    checkLevelUp();
  }
...
function checkStreak() {
  ...
  if (
    GS.lastSessionDate &&
    GS.lastSessionDate !== new Date().toDateString() &&
    GS.lastSessionDate !== yesterday
  ) {
    GS.streak = 0;
  }
}
...
function setupVisibilityAPI() {
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) {
      if (GS.timerRunning && GS.timerPhase === "focus")
        GS.awayStartTime = Date.now();
    } else {
      if (GS.awayStartTime) {
...
function applyPenalties(awaySecs) {
  ...
  GS.farmPlots.forEach((plot, i) => {
    ...
    if (rule === "freeze") {
      ...
      penalties.push({
        name: plant.name,
        emoji: plant.emoji,
        effect: `Qualidade: -${Math.floor(loss)}%`,
      });
      ...
      penalties.push({
        name: plant.name,
        emoji: plant.emoji,
        effect: `Progresso voltou ${Math.floor(reg * 10) / 10}min`,
      });
    } else if (rule === "die") {
      GS.farmPlots[i] = null;
      penalties.push({
  ...
  if (penalties.length > 0) {
    document.getElementById("penalty-modal-time").textContent =
      `Você ficou ${awaySecs}s fora do app durante uma sessão ativa.`;
    document.getElementById("penalty-list").innerHTML = penalties
      .map(
        (p) =>
