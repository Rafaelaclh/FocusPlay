...
async function exchangeCodeForToken(code, verifier) {
  ...
  if (!res.ok) { const e = await res.json(); throw new Error(e.error_description || 'Falha ao obter token'); }
  return res.json();
}
...
function spotifyInitSDK() {
  ...
  spotifyPlayer.addListener('player_state_changed', state => {
    ...
    if (!spotifyIsPlayingContext && state.paused && state.position === 0 &&
  ...
  spotifyPlayer.addListener('authentication_error', () => {
    ...
    spotifyAccessToken = null; spotifySDKInited = false;
    localStorage.removeItem(SPOTIFY_LS_TOKEN_KEY);
...
async function spotifyLoadRecommendations() {
  ...
  try {
    ...
    spotifyRenderQueue();
    if (typeof showToast === 'function') showToast('🎵 Recomendações adicionadas à fila!', 'success');
...
window.spotifyToggleQueue = function() {
  ...
  document.getElementById('sp-queue-btn').classList.toggle('active', spotifyQueueVisible);
...
function spotifyRenderQueue() {
  ...
  if (!spotifyQueue.length) {
    list.innerHTML = '<div class="sp-queue-empty">Fila vazia — toque uma música ou playlist</div>';
    return;
  }
  list.innerHTML = spotifyQueue.map((t, i) => {
...
function spotifyUpdateNowPlaying(track, isPlaying, position, duration) {
  ...
  if (duration) {
    document.getElementById('sp-progress-bar').style.width = (position / duration * 100) + '%';
...
function spotifyStartProgressTick(position, duration) {
  ...
  spotifyProgressInterval = setInterval(() => {
    ...
    if (current >= duration) { spotifyStopProgressTick(); return; }
...
window.spotifySeek = function(e) {
  ...
  spotifyPlayer.seek(Math.floor(((e.clientX - rect.left) / rect.width) * spotifyDuration));
};
...
window.spotifyTogglePlay = function() {
  ...
  spotifyPlayer.togglePlay().then(() => {
    spotifyPlayer.getCurrentState().then(state => {
      ...
      if (!state.paused) spotifyStartProgressTick(state.position, state.duration);
      else { spotifyStopProgressTick(); _progressPosition = state.position; }
    });
  });
};
...
window.spotifyDisconnect = function() {
  ...
  spotifyAccessToken = null; spotifyDeviceId = null;
  spotifySDKInited = false; spotifyQueue = []; spotifyQueueIndex = -1;
  localStorage.removeItem(SPOTIFY_LS_TOKEN_KEY);
  document.getElementById('spotify-connect-section').style.display = '';
  document.getElementById('spotify-player-section').style.display  = 'none';
